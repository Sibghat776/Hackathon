  ...
  try {
    ...
    if (!GEMINI_API_KEY) {
      return next(
        createError(
          500,
          "Server configuration error: Gemini API Key not found.",
        ),
      );
    }
    ...
    if (!text && !file) {
      return next(
        createError(
          400,
          "Barah-e-mehrbani, analysis ke liye text ya report ki tasveer faraham karein.",
        ),
      );
    }
    ...
    if (file) {
      ...
      const summary =
        result.response.text() || "Analysis ke liye koi jawab hasil nahi hua.";
    ...
    // Only Text Case
    const prompt = `Response Mujhe Text Clear Aur Formal Aur Simple Humanitize Language mein Chahiye. User ki taraf se di gayi ma'alumaat: "${text}". Is beemari ya condition ka hal aur agle zaroori iqdamaat bataiye. Agar Text English mein hai toh uska Answer English mein de, warna urdu mein.`;
    ...
    next(
      createError(
        500,
        "Gemini se analysis mein nakami. Barahe-meherbani dobara koshish karein.",
      ),
    );
  }
};
